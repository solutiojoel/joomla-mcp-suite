...
InMemoryTransport, fresh linked pair per call) when `INPROCESS_DOWNSTREAMS=1`
...
- Downstream module-level session caches (Joomla/Gantry logins) survive
  ...
  instances are cheap.
- `STDIO_CHILDREN` is currently empty — every downstream is a Node module. The
  stdio path is kept for any future non-Node downstream. Such a child must NOT
  see HTTP_PORT/PORT in env or it starts in HTTP mode; a failed stdio call
  invalidates the cached client so the next call respawns the child.
