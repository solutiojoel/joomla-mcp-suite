...
| Reachability | **Cloudflare Tunnel** — `cloudflared` runs as a Windows service on the box and holds an *outbound-only* connection to Cloudflare; `https://dashboard.<company-domain>` routes through it to `localhost:18310`. No port forwarding, no inbound firewall holes, no public IP exposure; HTTPS terminates at Cloudflare for free. Prereq: the domain's DNS (or a delegated subdomain) lives in a Cloudflare account |
