...
```
   Employee browser (office or home — any device on the tailnet)
        │
        │  http://<box>.tailXXXX.ts.net:18310        (WireGuard-encrypted by Tailscale)
...
| Reachability | **Tailscale** — box joins the existing tailnet; users hit its MagicDNS name. No port forwarding, no public DNS, no exposure. WireGuard encryption makes plain HTTP acceptable for MVP (`tailscale cert` can add HTTPS later) |
| Process supervision | **Windows Task Scheduler**: at-boot + on-failure restart tasks wrapping the existing start scripts + `start-agent-runtime.ps1` |
...
| Network hygiene | All service ports (9300–9306, 3506, 3507, 18305) bound/firewalled to localhost + the Tailscale interface; only :18310 is meant for humans |
| Updates | Remote in over Tailscale: `git pull`, `npm run build`, restart tasks |
| Replit constraint | Replit servers are **not** on the tailnet, so the hosted Replit app cannot call the API. The frontend is developed on Replit against the API doc and *deployed* onto the box; the frontend dev tests live by installing Tailscale |
...
| **7 plaintext orchestrator tokens** in `config/users.json` | Phase 0: store sha256 hashes, rotate all tokens, add `scripts/hash-tokens.js`. Lower urgency while tailnet-only, still first |
...
| **Unauthenticated internal ports** (3507 monitor, 18305 legacy dashboard, all MCP ports) | Firewall to localhost/tailnet; human access only via the runtime's authenticated proxy |
...
| **Tailnet** | The company's private Tailscale network; the only way to reach the box |
