...
## Phase 2 — Chat sessions (the core) — ✅ shipped 2026-07-15

> **Status:** landed and verified end-to-end against a live throwaway orchestrator + real SDK loops (Haiku): 40+ acceptance checks across four suites — sessions CRUD/limits/validation, bootstrap with real orchestrator tool calls (incl. a member's `allowedAgents` switch to content-build under their own bearer token), streaming text deltas + tool rows, 409 busy, interrupt (stops the turn, session stays usable), two users chatting concurrently with zero cross-talk, `Last-Event-ID` replay (verified across a runtime restart), kill-runtime-mid-conversation → next message resumes with context intact, per-user (2) and global (1-loop test cap) limits, and the active→idle→closed→resume lifecycle with `session.closed { idle_timeout }`.
> Notes from the field, baked into the driver: (1) the engine merges all queued input into one turn — a message sent to a just-resumed session rides along with the bootstrap turn and produces a single `result`, so turn tracking is a busy *flag*, not a per-message counter; (2) on a cold spawn the first turn can run before the orchestrator MCP connection registers — if bootstrap completes without a successful orchestrator tool call it is retried once automatically. Deviations from the sketch: built-in `Read` is enabled (only) so attached files work in chat; `siteUrl` is optional at session create (support tickets derive the site later); transcripts are a runtime-local sync-append JSONL module rather than `@solutio/logging` (ordering with seq assignment matters). Tunables in `.env.example` (`AGENT_RUNTIME_CHAT_MODEL`, `_MAX_TURNS`, `_MAX_SESSIONS_PER_USER`, `_MAX_GLOBAL_LOOPS`, `_IDLE_MINUTES`, `_CLOSE_MINUTES`, `AGENT_RUNTIME_DATA_DIR`).
...
## Phase 3 — Jobs, files, knowledge proxy, run monitor — ✅ shipped 2026-07-15

> **Status:** landed and verified with 49 live acceptance checks across three suites against a full rig (real joomla-mcp :9300 + agents-mcp :3506 + throwaway orchestrator :9392 + mock run monitor): catalog job cards with `runtime-file-id` schemas (hidden entries excluded), file upload/download with type/size/ownership enforcement, knowledge proxy CRUD against the **real** gateway (create/get/PUT→PATCH/delete/audit, unknown collection → 404), runs proxy field mapping (`runId→id`, `agentName→agent`), an `agent_ping` job end-to-end with **9 progress heartbeats relayed through the orchestrator**, per-user (1) and global (cap) job limits, FIFO queue with stop-while-queued, a deterministic `derive_content_schematic` job producing a workspace artifact **plus a gateway reference record (KB bridge A)**, stop-of-running-job proxied to the monitor, restart recovery (orphaned jobs → failed with explanatory error), and job-stream `Last-Event-ID` replay that closes after `done`.
> **Enabler shipped alongside:** the orchestrator now relays downstream `notifications/progress` to its own caller when the caller sent a progressToken (`callDownstream(…, onprogress)` + `extra.sendNotification` in the CallTool handler) — without this, no heartbeats cross the orchestrator hop and the runtime's `resetTimeoutOnProgress` would never fire. **Restart the live :9302 orchestrator to pick this up.**
> Notes from the field: the job worker must prepare its fresh MCP session before the tool call — `get_current_agent` → conditional `switch_agent` to the catalog entry's scope → `set_active_site` — because the orchestrator injects *its session's* active site into agents-mcp calls and enforces the current agent's tool allowlist. agents-mcp progress notifications carry no run id, so `runId` is resolved by matching the monitor's run list (sub-agent name + started-after + unclaimed). Deviations from the sketch: uploads live at `data/uploads/<fileId>/<name>` (backed up with the rest of `data/`, outside the static dir) rather than `workspace/uploads`; `agent_ping` ships in the catalog as a hidden admin-only smoke-test job. Operational: the runtime needs `KNOWLEDGE_GATEWAY_API_KEY`/`_BASE_URL` in its env (today the key only lives in `apps/knowledge-gateway-mcp/.env`). Tunables: `AGENT_RUNTIME_MAX_GLOBAL_JOBS` (2), `AGENTS_DASHBOARD_URL`.
